...
description: 'Configure ESLint for React, Vue, Svelte, Astro, Solid, Next.js, and Expo projects'
---
...
Svelte support includes Svelte 4+ template linting.

<Steps>
  <Step title="Enable Svelte support">
    ```js eslint.config.js
    import { zayne } from "@zayne-labs/eslint-config";
...
For SvelteKit projects, the configuration works automatically:
...
export default zayne({
  svelte: true,
  typescript: true, // SvelteKit uses TypeScript
});
```
...
<AccordionGroup>
  ...
  <Accordion title="SvelteKit with TanStack Query">
    ```js eslint.config.js
    import { zayne } from "@zayne-labs/eslint-config";
